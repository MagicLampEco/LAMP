...
> Allocation 18-pot (v17). **Đơn vị: NGHÌN LAMP** (1.000 LAMP); tổng = 36.000.000 nghìn = **36 tỷ LAMP**.
...
**Kiểm chứng:** tổng = **36 tỷ** ✓ · mọi pot trừ Reserve = 26,37 tỷ · Reserve = 9,63 tỷ · PhoenixKey 142.857 + TGE 21.143 nghìn = 164.000 nghìn (bù lẻ tròn).
> ⚠️ **Cap on-chain = MỘT trần duy nhất 36 tỷ** (`supply_state.cap`, bộ đếm `minted_total ≤ cap`). 26,37 / 9,63 là **phân-bổ** (Reserve so phần-còn-lại), KHÔNG phải 2 cap riêng on-chain — canonical KHÔNG tách quota dist/reserve.
...
**Trần chống loãng (KHÔNG dùng μ_pot):** **tổng MAGIC sinh từ các pot tổ chức mỗi epoch ≤ tổng MAGIC SnapshotGen
từ LAMP LƯU HÀNH cộng đồng (cùng epoch).** ⟹ MAGIC tổ chức không bao giờ vượt cộng đồng → tỷ trọng ≤ 50%, tự co.
Cơ chế: keeper tính 2 tổng → beacon → SnapshotGen vault-pot đọc beacon, scale δ theo `community/pot` nếu vượt
(dùng CHUNG keeper-beacon-circulating với Reserve). Chi tiết + tác động MAGIC-code: `LAMP-DISTRIBUTION-SPEC.md §4.1` (giao MAGIC-dev).
