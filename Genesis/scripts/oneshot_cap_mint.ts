...
//   • Có bước e: thu hồi Registry NFT (đốt), rồi đúc lại lần nữa để cho thấy
//     registry sống hay chết KHÔNG đổi kết quả — cổng đã chết vì số học.
//
...
async function main() {
  ...
  const metUnit = toUnit(nPid, MET_NAME);
  ...
  const genTx = await lucid.newTx()
    .mintAssets({ [threadUnit]: 1n, [regUnit]: 1n, [khoUnit]: 1n, [metUnit]: 1n })
  ...
  const regU = await findAt(walletAddr, regUnit);
  ...
  // ── e. thu hồi Registry NFT — cổng đã chết thì registry sống hay chết không đổi gì ──
  console.log(`\n── e. thu hồi Registry NFT (đốt) ──`);
  const burnTx = await lucid.newTx()
    .collectFrom([regU])
    .mintAssets({ [regUnit]: -1n })
    .attach.MintingPolicy(native)
    .complete();
  const bh = await (await burnTx.sign.withWallet().complete()).submit();
  console.log(`   TX: ${bh}\n   ${explorerTx(bh)}`); await lucid.awaitTx(bh); await sleep(20_000);
  const regGone = (await findAt(walletAddr, regUnit)) === undefined;
  console.log(`   ✓ Registry NFT ${regGone ? "đã biến mất khỏi chain" : "VẪN CÒN — kiểm lại"}`);
  if (!regGone) throw new Error("đốt Registry NFT không thành");

  const state = {
    network: NETWORK, nPid, lampPid, lampUnit, treHash, treAddr, claimHash,
    distCap: DIST_CAP.toString(), reserveCap: RESERVE_CAP.toString(), totalCap: TOTAL_CAP.toString(),
    genesisTx: gh, oneshotMintTx: mh, registryBurnTx: bh,
  ...
  console.log(`   • Registry NFT đã thu hồi — không còn thứ gì để xoay khoá`);
