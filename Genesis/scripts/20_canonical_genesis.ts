...
async function main(): Promise<void> {
  ...
  const tx = await lucid.newTx()
    ...
    // Registry ở VÍ trong màn diễn tập — chỉ đọc làm reference input, không bao giờ bị tiêu.
    // Mainnet đặt nó dưới `registry_write` do OrgDID/TAAD gác (mục D12); màn này KHÔNG
    // chứng minh được việc xoay khoá authority, xem runbook mục "cái này chưa chứng minh".
    .pay.ToAddressWithData(walletAddr, { kind: "inline", value: registryDatum(pkh) },
