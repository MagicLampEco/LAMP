// ISPO datum/redeemer codec — round-trip + Constr-shape khớp onchain types.ak.

import { describe, it, expect } from "vitest";
import { Constr, Data } from "@lucid-evolution/lucid";
import {
  ispoDatumToCbor, ispoDatumFromCbor, decodeIspoDatum,
...
describe("IspoDatum codec", () => {
  it("round-trips", () => {
    expect(ispoDatumFromCbor(ispoDatumToCbor(sampleDatum))).toEqual(sampleDatum);
  });

  it("encodes as Constr(0, [list, int, int, bytes, int])", () => {
    const back = decodeIspoDatum(Data.from(ispoDatumToCbor(sampleDatum)));
  ...
  it("epoch_roots rỗng round-trips", () => {
    ...
    expect(ispoDatumFromCbor(ispoDatumToCbor(d))).toEqual(d);
  });

  it("rejects wrong field count", () => {
    // Constr(0, []) → 0 fields → reject (cần 5).
    expect(() => decodeIspoDatum(Data.from("d87980"))).toThrow(/5 field/);
  });
});
