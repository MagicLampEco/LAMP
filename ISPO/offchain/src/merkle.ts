// ISPO Merkle (off-chain) — PHẢI khớp BYTE-PERFECT onchain merkle.ak.
//
...
export function hexToBytes(hex: string): Uint8Array {
  ...
  if (clean.length % 2 !== 0) throw new Error(`ISPO-MERKLE-000: hex độ dài lẻ: ${hex}`);
...
export function intToBe8(n: bigint): Uint8Array {
  if (n < 0n) throw new Error(`ISPO-MERKLE-001: int âm không hỗ trợ: ${n}`);
  if (n >= 1n << 64n) throw new Error(`ISPO-MERKLE-002: int vượt 2^64: ${n}`);
...
export class MerkleTree {
  ...
  constructor(leaves: MerkleLeaf[]) {
    if (leaves.length === 0) throw new Error("ISPO-MERKLE-010: cây rỗng");
    this.leaves = leaves;
  ...
  proofAt(index: number): MerkleStep[] {
    if (index < 0 || index >= this.leaves.length) {
      throw new Error(`ISPO-MERKLE-011: index ngoài phạm vi: ${index}`);
    }
  ...
  proofFor(epoch: bigint, owner: string): MerkleStep[] {
    ...
    if (i < 0) throw new Error(`ISPO-MERKLE-012: không thấy leaf (epoch=${epoch}, owner=${owner})`);
