// demo_ispo_claim.ts — RESUME bước I3 Claim của demo ISPO (I1/I2 đã on-chain).
...
//
// Dùng: tsx demo_ispo_claim.ts <genesisTxHash> <genesisIdx>
...
import { decodeIspoDatum, ispoDatumToCbor, claimRedeemerToCbor, burnSlotRedeemerToCbor } from "../offchain/src/datum.js";
...
if (!genesisTxHash || genesisIdxStr === undefined) { console.error("dùng: tsx demo_ispo_claim.ts <genesisTxHash> <genesisIdx>"); process.exit(1); }
...
const ispoNftPolicy: MintingPolicy = { type: "PlutusV3", script: applyParamsToScript(get("ispo_nft.ispo_nft.mint"), [genesisRef]) };
const ispoNftPolicyId = mintingPolicyToId(ispoNftPolicy);
const markerScript: Validator = { type: "PlutusV3", script: applyParamsToScript(get("ispo_marker.ispo_marker.spend"), [ispoNftPolicyId]) };
const markerHash = validatorToScriptHash(markerScript);
const poolScript: Validator = { type: "PlutusV3", script: applyParamsToScript(get("ispo_pool.ispo_pool.spend"), [ispoNftPolicyId, LAMP_POLICY, LAMP_NAME, [pkh], 1n, markerHash]) };
...
console.log(`[claim] nftPid:     ${ispoNftPolicyId}`);
...
const datumAfter: IspoDatum = { ...datum, distributed_total: datum.distributed_total + MY_OIL };
...
const tx = await lucid.newTx()
  ...
  .pay.ToAddressWithData(poolAddress, { kind: "inline", value: ispoDatumToCbor(datumAfter) }, poolOutAssets)
...
const out = {
  network: "Preview", demo: "ISPO", step: "I3_claim_resume",
  hash: h, link: link(h), poolAddress, markerAddress, ispoNftPolicyId,
...
await writeFile(resolve(import.meta.dirname, "demo-ispo-claim-out.json"),
  JSON.stringify(out, null, 2) + "\n");
console.log("DONE. wrote demo-ispo-claim-out.json");
