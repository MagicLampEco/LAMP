// demo_ispo.ts — DEMO ISPO trên Preview (bản HARDENING slot spend-once).
//
// Tx I1 Deploy: mint POOL NFT one-shot (ispo_nft MintPool) + seed pool tLAMP,
...
import { ispoDatumToCbor, mintPoolRedeemerToCbor } from "../offchain/src/datum.js";
...
import type { IspoDatum, ClaimProof } from "../offchain/src/types.js";
...
const out: Record<string, unknown> = { network: "Preview", demo: "ISPO", txs: [] as unknown[] };
...
console.log(`[ispo] ví: ${myAddr}`);
console.log(`[ispo] pkh (owner/admin): ${pkh}`);
console.log(`[ispo] tLAMP unit: ${lampUnit}`);
...
console.log(`[ispo] genesis ref: ${genesis.txHash}#${genesis.outputIndex}`);

const ispoNftPolicy: MintingPolicy = {
  type: "PlutusV3",
  script: applyParamsToScript(get("ispo_nft.ispo_nft.mint"), [genesisRef]),
};
const ispoNftPolicyId = mintingPolicyToId(ispoNftPolicy);

// marker (registry) — param chỉ policyId.
const markerScript: Validator = {
  type: "PlutusV3",
  script: applyParamsToScript(get("ispo_marker.ispo_marker.spend"), [ispoNftPolicyId]),
};
...
const poolScript: Validator = {
  type: "PlutusV3",
  script: applyParamsToScript(get("ispo_pool.ispo_pool.spend"),
    [ispoNftPolicyId, LAMP_POLICY, LAMP_NAME, ADMIN, ADMIN_THRESHOLD, markerHash]),
};
...
console.log(`[ispo] nftPolicyId: ${ispoNftPolicyId}`);
console.log(`[ispo] poolAddr:    ${poolAddress}`);
console.log(`[ispo] markerAddr:  ${markerAddress}`);
console.log(`[ispo] markerHash:  ${markerHash}`);
Object.assign(out, { ispoNftPolicyId, poolAddress, markerAddress, markerHash, lampUnit, poolNftUnit });
...
console.log(`[ispo] root0: ${root0}, pool seed: ${poolSeedOil} oil`);

// ── Datum khởi tạo (treasury_dest = payment-cred hash ví mình) ──────────────
const initDatum: IspoDatum = {
  epoch_roots: [],
...
let i1Hash: string;
{
  const tx = await lucid.newTx()
    ...
    .pay.ToAddressWithData(
      poolAddress,
      { kind: "inline", value: ispoDatumToCbor(initDatum) },
...
let i2Hash: string;
{
  ...
  const { tx, epoch, slotUnits, summary } = await buildSetRootTx({
    ...
    poolUtxo, ispoPoolScript: poolScript, ispoMarkerScript: markerScript,
    ispoNftPolicy, ispoNftPolicyId,
    root: root0, owners,
    adminSigners: ADMIN,
  });
  console.log("\n" + summary + "\n");
...
const mySlotUnit = toUnit(ispoNftPolicyId, sName);
let i3Hash: string;
let ownerAddrOut = "";
{
  const poolUtxos = await lucid.utxosAt(poolAddress);
  ...
  const { tx, amount, poolAfter, slotUnit, ownerAddress, summary } = await buildClaimTx({
    lucid, network: "Preview" as never,
    poolUtxo, slotUtxo, ispoPoolScript: poolScript, ispoMarkerScript: markerScript,
    ispoNftPolicy, ispoNftPolicyId,
...
await writeFile(resolve(import.meta.dirname, "demo-ispo-out.json"),
  JSON.stringify(out, (_k, v) => typeof v === "bigint" ? v.toString() : v, 2) + "\n");
console.log("\nDONE. wrote demo-ispo-out.json");
