...
LAMP, `1 LAMP = 10^6 oildrop` — `Utils`: `OILDROP_PER_LAMP = 1_000_000`). KHÔNG dùng số thực ở mọi nơi
ảnh hưởng value (nguyên tắc `Utils`: "ALL arithmetic BigInt. No Number for oildrop/nanogic/Q values").
...
INV-COLLECT dùng `==` (§2.3 — vá lần 2 F9; trước là `≥`) kết hợp TOTAL-CONSERVE (Định lý 1) ⟹ không oildrop nào
...
với:
- `S_total(LAMP) = 36×10^15 oildrop` **cố định tuyệt đối** (`Utils.S_LAMP_TOTAL`).
...
- **`Distribution/onchain/lib/magiclamp/lampdist/math.ak`** — `ceil_div`, `clamp` (tái dùng cho làm tròn an
  toàn-hệ-thống).
- **`Utils` (`src/index.ts`)** — `Q=10^9`, `mulQ`, `clamp`, `S_LAMP_TOTAL`, `OILDROP_PER_LAMP`,
