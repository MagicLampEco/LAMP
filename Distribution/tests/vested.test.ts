...
describe("vested(t) = min(E, D·dpe·max(0,t−t0))", () => {
  it("trước/đúng t0 → 0 (max(0, ·))", () => {
    expect(vested(lampOil(1000n), D, 1n, 5n, 5n)).toBe(0n);   // t == t0
    expect(vested(lampOil(1000n), D, 1n, 5n, 3n)).toBe(0n);   // t < t0
  });
  ...
  it("cap E: không vượt entitlement dù t lớn", () => {
    ...
    expect(vested(lampOil(1000n), D, 1n, 0n, 11n)).toBe(lampOil(1000n)); // vượt → vẫn E
  ...
  it("đơn điệu tăng theo t (không bao giờ giảm)", () => {
    ...
    expect(prev).toBe(lampOil(1000n)); // đạt cap cuối cùng
  });
  ...
  it("E > D → nhỏ giọt D/epoch tới hết, ⌈E/D⌉ epoch", () => {
    ...
    expect(vested(E, D, 1n, 0n, 1n)).toBe(lampOil(100n));
    expect(vested(E, D, 1n, 0n, 2n)).toBe(lampOil(200n));
  ...
  it("drops_per_epoch > 1 → mở nhanh hơn (hook DAO)", () => {
    ...
    expect(epochsToFull(lampOil(1000n), D, 2n)).toBe(5n); // ⌈1000/200⌉
  });
  ...
  it("entitlement bảo toàn: bỏ lỡ epoch không mất quyền (vested cộng dồn)", () => {
    ...
    const E = lampOil(1000n);
    expect(vested(E, D, 1n, 0n, 5n)).toBe(lampOil(500n));
  });
});
...
describe("epochsToFull = ⌈E / (D·dpe)⌉", () => {
  ...
  it("ceil khi lẻ", () => {
    expect(epochsToFull(lampOil(250n), D, 1n)).toBe(3n);   // ⌈2.5⌉
    expect(epochsToFull(lampOil(1n), D, 1n)).toBe(1n);     // ví siêu nhỏ
  });
