// Airdrop Merkle — TÁI DÙNG nguyên cơ chế Merkle canonical của LampDistribution
// (lottery design, đã audit). PHẢI khớp byte-perfect với onchain merkle.ak:
// BLAKE2b-256, RFC 6962 domain separation, sorted-pair, leaf carry-up tầng lẻ.
//
//   leaf = blake2b256( 0x00 ‖ owner_pkh ‖ won_cumulative[be8] )
//   node = blake2b256( 0x01 ‖ min(a,b) ‖ max(a,b) )
//
// Claim airdrop permissionless: delegator/SPO xuất Merkle proof, validator đối chiếu
// root đã post lên beacon mỗi epoch (§6.2). won_cumulative cộng dồn ⇒ claim trễ vẫn đủ.

import { blake2b } from "@noble/hashes/blake2b";
import { MERKLE_LEAF_PREFIX, MERKLE_NODE_PREFIX } from "./constants.js";

// ── byte helpers (pure, no Buffer dependency) ──
 ...
 * Leaf sort theo leaf-hash ⇒ cây tất định bất kể thứ tự input. Tầng lẻ: node cuối
 * carry-up không đổi. Trả root + proof cho từng owner.
 */
