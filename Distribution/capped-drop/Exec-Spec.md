...
# DROP_VALUE_OILDROP=100000000         # 100 LAMP/drop·epoch (D) — default D_GENESIS
...
npm run genesis
# Tùy chọn:
# DROP_VALUE_OILDROP=100000000 npm run genesis   # D = 100 LAMP/drop·epoch (default D_GENESIS)
# TREASURY_FUND_OILDROP=1000000000000 npm run genesis  # 1_000_000 LAMP
```

Env vars:
- `DROP_VALUE_OILDROP` — D (oildrop); mặc định `D_GENESIS = 100_000_000` (100 LAMP).
- `TREASURY_FUND_OILDROP` — oildrop fund treasury; mặc định `500_000 × 10^6`.
...
```bash
npm run e2e
# Tùy chọn: DROP_VALUE_OILDROP=100000000 npm run e2e
```
...
```
Setup:  E = 1_000_000_000 oildrop (1000 LAMP), D = 100_000_000 oildrop (100 LAMP), dpe = 1, t0 = 100.
Epoch 103 (elapsed=3): vested = min(1000, 100·3) = 300 LAMP → redeem 300.
Kỳ vọng:
  - TX on-chain confirm.
  - ClaimAccount out: redeemed = 300_000_000, field khác unchanged.
  - Ví A nhận +300_000_000 oildrop LAMP.
  - Treasury giảm đúng 300_000_000 oildrop.
...
```
Setup:  E = 350_000_000 oildrop (350 LAMP), D = 100_000_000, dpe = 1, t0 = 100.
...
```
Setup:  E = 500_000_000 oildrop (500 LAMP), D = 100_000_000, dpe = 1, t0 = 100.
...
```
Setup:  E = 40_000_000 oildrop (40 LAMP), D = 100_000_000, dpe = 1, t0 = 100.
