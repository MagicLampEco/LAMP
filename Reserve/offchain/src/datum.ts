...
//
//   ReserveState{start_epoch, total_oil, drawn_oil} = Constr(0, [int, int, int])
...
// ── ReserveState ────────────────────────────────────────────────────────
// Constr(0, [start_epoch:int, total_oil:int, drawn_oil:int])

export function encodeReserveState(s: ReserveState): Constr<Data> {
  return new Constr(0, [s.start_epoch, s.total_oil, s.drawn_oil]);
}
...
export function decodeReserveState(d: Data): ReserveState {
  ...
  if (c.fields.length !== 3)
    throw new Error(`RDATUM-011: ReserveState expects 3 fields, got ${c.fields.length}`);
  return {
