...
import { RESERVE_TOTAL_OILDROP, MAX_PER_EPOCH, RELEASE_EPOCHS } from "../offchain/src/constants.js";
import type { ReserveState } from "../offchain/src/types.js";

const E = RESERVE_TOTAL_OILDROP; // 9_630_000_000_000_000n
...
describe("applyDraw — transition fail-fast", () => {
  const fresh: ReserveState = {
    start_epoch: S0, total_oildrop: E, drawn_oildrop: 0n, last_epoch: S0,
  };
  ...
  it("happy: kéo tối đa (mặc định = trần); drawn += trần; last_epoch := t", () => {
    ...
    expect(next.total_oildrop).toBe(E);
    expect(next.drawn_oildrop).toBe(fresh.drawn_oildrop + CAP);
  ...
  it("pot cạn → throw (không tx rỗng)", () => {
    const empty: ReserveState = {
      start_epoch: S0, total_oildrop: E, drawn_oildrop: E, last_epoch: S0 + 999n,
    };
