// SRCL datum/redeemer codec — Plutus Data (Lucid Evolution).
// PHẢI khớp byte-perfect onchain types.ak. Constr index = thứ tự khai báo.
//
//   SrclDatum  = Constr(0, [list<bytes>, int, int, bytes, int])
//   MerkleStep = Constr(0, [bytes, bool])        // bool: False=Constr0, True=Constr1
//   ClaimProof = Constr(0, [int, bytes, int, list<MerkleStep>])
//   SrclRedeemer:    SetRoot=Constr(0,[bytes]), Claim=Constr(1,[ClaimProof]), Sweep=Constr(2,[])
//   SrclNftRedeemer: MintPool=Constr(0,[]), MintSlots=Constr(1,[]), BurnSlot=Constr(2,[])

import { Constr, Data } from "@lucid-evolution/lucid";
import type { SrclDatum, ClaimProof, MerkleStep } from "./types.js";
...
function asConstr(d: Data, ctx: string): Constr<Data> {
  ...
  throw new Error(`SRCL-DATUM-000: expected Constr for ${ctx}`);
}

function asInt(d: Data, ctx: string): bigint {
  if (typeof d !== "bigint") throw new Error(`SRCL-DATUM-002: expected int for ${ctx}`);
  return d;
}

function asBytes(d: Data, ctx: string): string {
  if (typeof d !== "string") throw new Error(`SRCL-DATUM-003: expected bytes for ${ctx}`);
  return d;
}

function asList(d: Data, ctx: string): Data[] {
  if (!Array.isArray(d)) throw new Error(`SRCL-DATUM-004: expected list for ${ctx}`);
  return d;
}
...
function decodeBool(d: Data, ctx: string): boolean {
  ...
  if (c.index !== 0 && c.index !== 1) throw new Error(`SRCL-DATUM-005: bool index ${c.index} for ${ctx}`);
  return c.index === 1;
}
...
export function decodeMerkleStep(d: Data): MerkleStep {
  ...
  if (c.index !== 0) throw new Error(`SRCL-DATUM-010: MerkleStep expects Constr 0, got ${c.index}`);
  if (c.fields.length !== 2) throw new Error(`SRCL-DATUM-011: MerkleStep expects 2 fields, got ${c.fields.length}`);
...
export function decodeClaimProof(d: Data): ClaimProof {
  ...
  if (c.index !== 0) throw new Error(`SRCL-DATUM-020: ClaimProof expects Constr 0, got ${c.index}`);
  if (c.fields.length !== 4) throw new Error(`SRCL-DATUM-021: ClaimProof expects 4 fields, got ${c.fields.length}`);
  return {
...
export function encodeSrclDatum(d: SrclDatum): Constr<Data> {
  return new Constr(0, [
...
export function decodeSrclDatum(d: Data): SrclDatum {
  const c = asConstr(d, "SrclDatum");
  if (c.index !== 0) throw new Error(`SRCL-DATUM-030: SrclDatum expects Constr 0, got ${c.index}`);
  if (c.fields.length !== 5) throw new Error(`SRCL-DATUM-031: SrclDatum expects 5 fields, got ${c.fields.length}`);
  return {
...
export function srclDatumToCbor(d: SrclDatum): string {
  return Data.to(encodeSrclDatum(d));
}

export function srclDatumFromCbor(cbor: string): SrclDatum {
  return decodeSrclDatum(Data.from(cbor));
}

// ── SrclRedeemer (pool spend) ───────────────────────────────────────────
